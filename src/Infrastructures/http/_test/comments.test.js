...
describe('/threads/{threadId}/comments endpoint', () => {
  ...
  describe('when DELETE /threads/{threadId}/comments/{commentId}', () => {
    it('should response 200 if comment id and user id valid and authorized', async () => {
      // Arrange
      const server = await createServer(container);
      // add user
      await server.inject({
    ...
    it('should response 404 when comment is not aligned with thread', async () => {
      ...
      const { data: { addedThread: { id: firstThreadId } } } = JSON.parse(
        firstThreadResponse.payload,
      );
      // add second thread
      ...
      const { data: { addedThread: { id: secondThreadId } } } = JSON.parse(
        secondThreadResponse.payload,
      );
      // add comment on second thread
      const commentResponse = await server.inject({
        method: 'POST',
        url: `/threads/${secondThreadId}/comments`,
        payload: {
          content: 'comment content',
        },
        headers: {
      ...
      const response = await server.inject({
        method: 'DELETE',
        url: `/threads/${firstThreadId}/comments/${commentId}`,
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
    ...
    it('should response 403 when user id not matched with comment owner', async () => {
      // Arrange
      const server = await createServer(container);
      // add first user
      await server.inject({
